...
void ADC_Init(uint32_t channelNum) {
	...
	// three delay statement is required for the system to run (idk why)
...
void ADC_Collect(uint32_t channelNum, uint32_t fs, void (*task)(uint32_t)) {
	...
	TIMER2_TAILR_R = 80000000/fs-1;    // fs Hz sampling rate, 80 MHz clock
	...
	ADC0_EMUX_R = (ADC0_EMUX_R&0xFFFFF0FF)+0x0500; // timer trigger event for SS2
...
//	 delay = SYSCTL_RCGCTIMER_R;   // allow time to finish activating
//	TIMER0_CTL_R = 0x00000000;    // disable timer2A during setup
//	TIMER0_CFG_R = 0;             // configure for 32-bit timer mode
//	TIMER0_TAMR_R = 0x00000002;   // configure for periodic mode, default down-count settings
//	TIMER0_TAPR_R = 0;            // prescale value for trigger
//	TIMER0_TAILR_R = 80000000/fs-1;    // fs Hz sampling rate, 80 MHz clock
//	TIMER0_IMR_R = 0x00000000;    // disable all interrupts
//	TIMER0_CTL_R |= 0x00000020;   // enable timer2A trigger to ADC
//	TIMER0_CTL_R |= 0x00000001;   // enable timer2A 32-b, periodic, no interrupts
//	// timer is used to count down, ADC ISR is triggered
//	ADC0_PC_R = 0x01;         // configure for 125K samples/sec
//	ADC0_SSPRI_R = 0x3210;    // sequencer 0 is highest, sequencer 3 is lowest
//	ADC0_ACTSS_R &= ~0x04;    // disable sample sequencer 2
