...
#define NVIC_ST_RELOAD_M        0x00FFFFFF  // Maximum Counter load value; 24 bit

// Initialize SysTick with busy wait running at bus clock.
void SysTick_Init(void){
  NVIC_ST_CTRL_R = 0;                   // disable SysTick during setup
  NVIC_ST_RELOAD_R = NVIC_ST_RELOAD_M;  // maximum reload value
  NVIC_ST_CURRENT_R = 0;                // any write to current clears it; current decrements 1 each clock cycle
...
void SysTick_Wait(uint32_t delay){
  ...
  do {
    elapsedTime = (startTime - NVIC_ST_CURRENT_R) & 0x00FFFFFF;  // even with underflow, this gives the correct result (elapsed time)
  }
...
void SysTick_Wait10ms(uint32_t delay){
  uint32_t i;
  for(i=0; i<delay; i++){
    SysTick_Wait(800000);  // wait 10ms (assumes 80 MHz clock); this number must be less than NVIC_ST_RELOAD_M
  }
}
